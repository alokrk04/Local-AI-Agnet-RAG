...
### 1. Prerequisites
You will need to have [Ollama](https://ollama.com/) installed on your machine to run the local models. 
...
**What happens next?**
1. **First Run:** The script (`vector.py`) will automatically detect if the ChromaDB database exists. If not, it will read `realistic_restaurant_reviews.csv`, chunk the data, generate embeddings using `mxbai-embed-large`, and save them to a local `./chrome_langchain_db` folder.
2. **Chat Loop:** The agent will boot up and prompt you to ask questions. You can ask things like:
...
3. **Type `q`** at any time to exit the chat.
...
## 🛠️ How It Works
1. **Data Ingestion:** `vector.py` uses Pandas to read the restaurant reviews. It concatenates the Title and Review text, and stores the Rating and Date as searchable metadata.
2. **Vectorization:** These documents are embedded using `mxbai-embed-large` and stored locally via Chroma.
